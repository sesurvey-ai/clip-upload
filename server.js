...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    html,body{min-height:100vh;background:linear-gradient(160deg,#0a0a1a 0%,#111128 40%,#1a1040 100%);font-family:'Prompt',sans-serif;color:#e2e0ef}
    input::placeholder{color:#625d80}
    @keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}
    @keyframes spin{to{transform:rotate(360deg)}}
    @keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}
    @keyframes pop{0%{transform:scale(.9);opacity:0}100%{transform:scale(1);opacity:1}}
    .card{animation:fadeUp .5s ease-out both}
    .drop-zone{border:2px dashed #2e2952;border-radius:18px;padding:40px 20px;text-align:center;cursor:pointer;transition:all .25s}
    .drop-zone:hover{border-color:#4a3d8f;background:rgba(106,74,255,0.03)}
    .drop-zone.active{border-color:#6d4aff!important;background:rgba(106,74,255,0.07)!important}
    ...
    .btn-main{background:linear-gradient(135deg,#6d4aff 0%,#9333ea 50%,#6d4aff 100%);background-size:200% 100%;color:#fff;padding:14px;width:100%;font-size:15px}
    .btn-main:hover:not(:disabled){background-position:100% 0;box-shadow:0 8px 32px rgba(109,74,255,.35);transform:translateY(-1px)}
    .btn-main:disabled{opacity:.45;cursor:not-allowed;transform:none;box-shadow:none}
    .btn-ghost{background:transparent;border:1.5px solid #2e2952;color:#9690b3;padding:8px 18px;font-size:13px}
    .btn-ghost:hover{border-color:#6d4aff;color:#b4a8ff}
    .progress-track{height:6px;background:#1e1b38;border-radius:6px;overflow:hidden}
    .progress-fill{height:100%;border-radius:6px;background:linear-gradient(90deg,#6d4aff,#9333ea,#6d4aff);background-size:200% 100%;animation:shimmer 1.5s linear infinite;transition:width .3s ease}
    ...
    .tag-ok{background:rgba(52,211,153,.1);color:#34d399}
    .tag-fail{background:rgba(248,113,113,.1);color:#f87171}
    .tag-up{background:rgba(109,74,255,.1);color:#a78bfa}
    .folder-input{width:100%;padding:13px 16px;font-size:15px;background:rgba(30,27,56,0.6);border:1.5px solid #2e2952;border-radius:12px;color:#e2e0ef;outline:none;transition:border-color .2s,box-shadow .2s;font-family:'Prompt',sans-serif}
    .folder-input:focus{border-color:#6d4aff;box-shadow:0 0 0 3px rgba(109,74,255,.12)}
    .file-list{max-height:280px;overflow-y:auto}
    .file-list::-webkit-scrollbar{width:4px}
    .file-list::-webkit-scrollbar-track{background:transparent}
    .file-list::-webkit-scrollbar-thumb{background:#2e2952;border-radius:10px}
    .file-row{display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:12px;margin-bottom:5px;background:rgba(30,27,56,0.5);animation:fadeUp .3s ease-out both}
    .file-thumb{width:40px;height:40px;border-radius:8px;object-fit:cover;border:1px solid #2e2952;flex-shrink:0}
    .file-icon{width:40px;height:40px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:20px;background:rgba(109,74,255,.08);border:1px solid #2e2952;flex-shrink:0}
    .file-name{font-size:13px;font-weight:500;color:#dbd8ef;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .file-size{font-size:11px;color:#4a4570}
    .remove-btn{background:none;border:none;color:#4a4570;cursor:pointer;font-size:18px;padding:2px 6px;border-radius:6px;transition:color .2s;flex-shrink:0}
    .remove-btn:hover{color:#f87171}
    .spinner{width:12px;height:12px;border:2px solid #a78bfa;border-top-color:transparent;border-radius:50%;animation:spin .7s linear infinite;display:inline-block}
    .spinner-lg{width:16px;height:16px;border:2px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin .8s linear infinite;display:inline-block}
    .error-box{padding:10px 14px;border-radius:12px;margin-bottom:18px;background:rgba(248,113,113,.06);border:1px solid rgba(248,113,113,.18);color:#f87171;font-size:13px;line-height:1.6}
    .result-box{padding:22px;border-radius:16px;background:rgba(52,211,153,.04);border:1px solid rgba(52,211,153,.12);animation:pop .35s ease-out}
...
<div style="padding:36px 24px 0;max-width:600px;margin:0 auto;">
  <div style="display:flex;align-items:center;gap:14px;">
    <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#6d4aff,#9333ea);display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 6px 24px rgba(109,74,255,.3);">☁️</div>
    <div>
      <h1 style="font-size:24px;font-weight:700;letter-spacing:-.03em;background:linear-gradient(135deg,#fff 20%,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">Drive Uploader</h1>
      <p style="font-size:13px;color:#625d80;font-weight:300;">อัปโหลดไฟล์ลงเครื่อง · ไม่จำกัดขนาด · ไม่ต้องล็อกอิน</p>
    </div>
  </div>
</div>

<div class="card" style="max-width:600px;margin:24px auto 32px;padding:28px;background:rgba(20,18,44,0.8);backdrop-filter:blur(24px);border-radius:22px;border:1px solid rgba(109,74,255,.1);box-shadow:0 24px 64px rgba(0,0,0,.35);">

  <div style="margin-bottom:22px;">
    <label style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:500;color:#9690b3;margin-bottom:8px;">
    ...
    <p style="font-size:11px;color:#4a4570;margin:6px 0 0 4px;">ไฟล์จะถูกบันทึกที่ ./uploads/ชื่อโฟลเดอร์/</p>
  </div>
  ...
  <div id="dropZone" class="drop-zone" style="margin-bottom:22px;">
    ...
    <p style="font-weight:500;font-size:15px;color:#b4a8ff;">ลากไฟล์มาวางที่นี่</p>
    <p style="font-size:13px;color:#4a4570;margin-top:6px;">หรือคลิกเพื่อเลือก · ทุกประเภท ไม่จำกัดขนาด</p>
  </div>
  ...
  <div id="fileListHeader" class="hidden">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding:0 2px;">
      <span id="fileCount" style="font-size:13px;color:#9690b3;font-weight:500;"></span>
  ...
  <div id="progressSection" class="hidden" style="margin-bottom:20px;">
    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px;">
      <span id="progressText" style="font-size:13px;color:#a78bfa;"></span>
      <span id="progressPercent" style="font-size:20px;font-weight:700;color:#a78bfa;"></span>
    </div>
    <div class="progress-track">
      <div id="progressBar" class="progress-fill" style="width:0%;"></div>
    </div>
    <div id="progressDetail" style="font-size:11px;color:#4a4570;margin-top:6px;"></div>
  </div>
  ...
  <div id="resultBox" class="result-box hidden">
    <div style="text-align:center;margin-bottom:16px;">
      ...
      <div id="resultSummary" style="font-size:13px;color:#9690b3;margin-top:4px;"></div>
    </div>
...
<div style="text-align:center;padding:0 24px 36px;font-size:11px;color:#2e2952;">
...
async function handleUpload(){
  ...
  setTimeout(function(){
    ...
    if(ok===total){resultEmoji.textContent='🎉';resultTitle.textContent='อัปโหลดสำเร็จทั้งหมด!';resultTitle.style.color='#34d399';}
    else if(ok>0){resultEmoji.textContent='⚠️';resultTitle.textContent='อัปโหลดสำเร็จบางส่วน';resultTitle.style.color='#fbbf24';}
    else{resultEmoji.textContent='❌';resultTitle.textContent='อัปโหลดล้มเหลว';resultTitle.style.color='#f87171';}
    resultSummary.textContent='สำเร็จ '+ok+'/'+total+' ไฟล์ → โฟลเดอร์ "'+folder+'"';

    resultListEl.innerHTML=results.map(function(r){
      var c=r.success?'#34d399':'#f87171',nc=r.success?'#dbd8ef':'#625d80';
      var extra=r.success?'<span style="color:#4a4570;font-size:11px;margin-left:auto;">'+fmtSize(r.size)+'</span>'
        :'<span style="color:#f87171;font-size:11px;margin-left:auto;" title="'+(r.error||'')+'">'+r.error+'</span>';
